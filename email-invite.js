...
+  if (isNaN(d)) throw new Error('Invalid start datetime');
+  const z = new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate(), d.getUTCHours(), d.getUTCMinutes(), 0, 0));
...
+  const organizerEmail = p.organizerEmail || process.env.ORGANIZER_EMAIL || process.env.DEMO_ORGANIZER_UPN;
...
+  const ics = [
+    'BEGIN:VCALENDAR','PRODID:-//Wave//Demo Invite//EN','VERSION:2.0','METHOD:REQUEST','BEGIN:VEVENT',
+    `UID:${uid}`,`DTSTAMP:${fmtICS(new Date())}`,`DTSTART:${fmtICS(startISO)}`,`DURATION:PT10M`,
+    `SUMMARY:${escICS(p.subject||'Wave Demo')}`,
+    `DESCRIPTION:${escICS(`Quick 10-minute Wave demo.${p.meetingLink?`\nJoin: ${p.meetingLink}`:''}`)}`,
...
+    alternatives: [{ contentType: 'text/calendar; method=REQUEST; charset=UTF-8', content: ics }],
+    attachments: [{ filename: 'invite.ics', content: ics, contentType: 'text/calendar; method=REQUEST; name="invite.ics"' }]
+  });
